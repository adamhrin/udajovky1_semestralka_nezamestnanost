...
namespace structures
{
	...
	public:
		...
		void sort(UnsortedSequenceTable<K, T>& table, Comparator<T>& comparator);
	};
	...
	inline void HeapSort<K, T>::sort(UnsortedSequenceTable<K, T>& table)
	{
		...
		for (int i = length - 1; i >= 1; i--)
		{
			table.swap(0, i);
			aktualny = 0;
			do // reorganizacia do max haldy, vybrany prvok ide na koniec
			{
				vymena = false;
				lavy = aktualny * 2 + 1;
				pravy = aktualny * 2 + 2;
