...
namespace structures 
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		{
			clear();
			for each (auto item in other)
			{
				list_->add(new TableItem<K, T>(item->getKey(), item->accessData()));
			}
		}
		return *this;
	}
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (item == nullptr)
		{
			throw std::logic_error("Item not in table!");
		}
	...
	inline void SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		if (containsKey(key))
		{
			throw std::logic_error("Item already in table!");
		}
