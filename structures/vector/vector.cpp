...
namespace structures {
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index in Vector!");
		DSRoutines::rangeCheckExcept(index + count, size_ + 1, "Memory overflow!");
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		...
		DSRoutines::rangeCheckExcept(destStartIndex + length, dest.size_ + 1, "Memory overflow!");
		DSRoutines::rangeCheckExcept(srcStartIndex + length, src.size_ + 1, "Memory overflow!");
