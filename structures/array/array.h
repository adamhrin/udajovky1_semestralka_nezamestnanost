...
namespace structures 
{
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		if (this != &other)
		{
			...
			throw std::logic_error("Array size differs!");
		}
		return *this;
		//throw std::logic_error("Array size differs!");
	}
	...
	{
		const int vectorSrcStartIndex = src.mapFunction(srcStartIndex);
		const int vectorDestStartIndex = dest.mapFunction(destStartIndex);
		//const int vectorLength = this.mapFunction(length);
		//try catch pre vynimku, aby nebol invalid index in vector ale invalid index in array
		/*try
		{*/
			Vector::copy(*src.vector_, vectorSrcStartIndex, *dest.vector_, vectorDestStartIndex, (length * sizeof(T)));
		/*}
		catch (const DSRoutines::rangeCheckExcept)
		{
